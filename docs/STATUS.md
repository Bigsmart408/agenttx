...
- Selective commit **via rollback-then-commit** works.
- **Gap confirmed:** naive `commit(up_to)` still materializes full overlay (`fs_selective=false`).
...
1. **True selective filesystem commit** — today `try commit` dumps whole upperdir; ledger frontier alone is not FS-selective. Need path-filtered commit or commit-from-restored snapshot.
...
3. **Lower overlay overhead** — shared pool ≈ per-call cost (~170–200ms/step); value is isolation/ledger, not speedup. Explore mergerfs / stacked `-L`, or longer-lived mounts without remount tax.
